...
const Playlist = () => {
    ...
    const deletePlaylist = async (playlist_id: number) => {
        try {
            ...
            const response = await fetch(`${REST_BASE_URL}/playlists/${playlist_id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': token ?? "Bearer " + token,
                },
            ...
            if (!response.ok) {
                ...
                setPlaylistData((prevData) => prevData.filter((item) => item.playlist_id !== playlist_id));
    ...
    const editPlaylist = async () => {
        ...
        try {
            const token = cookies.token;
            const response = await fetch(`${REST_BASE_URL}/playlists/` + String(playlist_id), {
                method: "PATCH",
                headers: {
                    'Authorization': token ?? "Bearer " + token,
                    'Content-Type': 'application/json',
            ...
            if (!response.ok) {
                ...
                setPlaylistData((prevData) => {
                    const newDataArray = prevData.map(item => (item.playlist_id === newData.data.playlist_id ? newData.data : item));
                    return newDataArray;
                });
                                        ...
                                        <Tr key={item.title}>
                                            ...
                                            <Td textAlign="center" verticalAlign="middle">
                                                <IconButton
                                                    ...
                                                    mr={2}
                                                    onClick={() => {
                                                        navigate(`/playlistdetails/${item.playlist_id}`, { state: { playlist: item } })
                                                    }}
                                                />
                                                <IconButton
                                                    icon={<EditIcon />}
                        ...
                        <ModalFooter textAlign="center" verticalAlign="middle">
                            ...
                            >
                                <Button onClick={() => {
                                    ...
                                    closeDeleteModal();
                                }} style={{color: "white"}} backgroundColor="red.400" w="50%">Delete</Button>
                    ...
                    <ModalContent>
                        ...
                        <ModalBody>
                            ...
                            <FormControl>
                                ...
                                <Textarea placeholder="Your playlist description" size="sm" value={description} onChange={(e) => setDescription(e.target.value)}/>
                            </FormControl>
                        ...
                        <ModalFooter>
                            <Button onClick={modalMode === "add" ? addPlaylist : editPlaylist} colorScheme='blue' mr={3}>
                                Save Playlist
                            </Button>
