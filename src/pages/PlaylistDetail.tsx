...
const PlaylistDetails = () => {
    ...
    const fetchPlaylistBook = async () => {
        ...
        const response = await fetch(`${REST_BASE_URL}/playlists/` + String(playlist.playlist_id) + '/books', {
            method: "GET",
            headers: {
                            ...
                            <Tbody>
                                {
                                    recommendedBooks.map((item) => (
                                        <Tr key={item.title}>
                                            ...
                                            <Td textAlign="center" verticalAlign="middle">
                                                {(item.release_date ? new Date(item.release_date).toISOString().slice(0, 10) : "No date available")}
                                            </Td>
                                            <Td textAlign="center" verticalAlign="middle">
                                                <IconButton
                                                    icon={<AddIcon />}
